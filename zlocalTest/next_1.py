...
class Module:
    ...
    @T.prim_func
    def long_lines(value_1: T.handle("float32", "global"), c_idx_1: T.handle("int32", "global"), idx_1: T.handle("int32", "global"), vector_in: T.handle("float32", "global"), res_t: T.handle("float32", "global"), row_num_real: T.int32, col_num_real: T.int32, col_num_split: T.int32, row_num_split: T.int32):
    ...
    def long_lines_kernel_3(res_val: T.handle("float32", "global"), row_num_real: T.int32):
        ...
        for threadIdx_x in T.thread_binding(1, thread="threadIdx.x"):
            ...
            threadIdx_x_1 = T.launch_thread("threadIdx.x", 256)
            if blockIdx_x * 256 + threadIdx_x_1 < row_num_real:
                res_val_1[blockIdx_x * 256 + threadIdx_x_1] = T.float32(0.0)
    ...
    def long_lines_kernel_3_1(buffer_1_idx: T.handle("int32", "global"), buffer_1_val: T.handle("float32", "global"), col_1_idx: T.handle("int32", "global"), res_val: T.handle("float32", "global"), vec_val: T.handle("float32", "global"), col_num_real: T.int32, col_num_split: T.int32, row_num_real: T.int32, row_num_split: T.int32):
        ...
        for threadIdx_x in T.thread_binding(1, thread="threadIdx.x"):
            ...
            for col_idx_0 in range((col_num_split + 127) // 128):
                if col_idx_0 * 128 + threadIdx_x_1 < col_num_split:
                    in_thread_res_local_1[0] = in_thread_res_local_1[0] + buffer_1_val_1[col_idx_0 * 128 + blockIdx_x * col_num_split + threadIdx_x_1] * vec_val_1[col_1_idx_1[col_idx_0 * 128 + blockIdx_x * col_num_split + threadIdx_x_1]]
            with T.attr(T.comm_reducer(lambda x0, y0: x0 + y0, [T.float32(0.0)]), "reduce_scope", T.reinterpret("handle", T.uint64(0))):
                ...
                red_buf0_1[0] = red_buf0_1[0] + t0_1[0]
                if threadIdx_x_1 % 32 == 0:
                    red_buf_staging[threadIdx_x_1 // 32] = red_buf0_1[0]
                T.tvm_storage_sync("shared")
                if threadIdx_x_1 < 4:
                    red_buf0[0] = red_buf_staging[threadIdx_x_1]
    ...
    @T.prim_func
    def parent_kernel(value_1: T.handle, c_idx_1: T.handle, idx_1: T.handle, vector_in: T.handle, res_t: T.handle, row_num_real: T.int32, col_num_real: T.int32, col_num_split: T.int32, row_num_split: T.int32):
        T.func_attr({"target": T.target({"arch": "sm_86", "host": {"keys": ["cpu"], "kind": "llvm", "mtriple": "x86_64-pc-linux-gnu", "tag": ""}, "keys": ["cuda", "gpu"], "kind": "cuda", "max_num_threads": 1024, "tag": "", "thread_warp_size": 32})})
